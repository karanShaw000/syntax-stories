...
class BlogController {
  ...
  static blogFeed = asyncCatch(
    async (req: IsUserLoggedInRequest, res: Response) => {
      ...
      if (isLoggedIn && userId) {
        ...
        res
          ...
          .json({
            message: "Blog Feed",
            data: { blogs: blogs, currentPage: page, totalPage: totalPage },
          });
      }
    }
  );
  ...
  static getUserBlogs = asyncCatch(async (req: IAuthRequest, res: Response) => {
    ...
    res
      ...
      .json({
        message: "User Blogs",
        data: { blogs: blogsWithFlag, currentPage: page, totalPage: totalPage },
      });
  });

  static getBlogById = asyncCatch(
    async (req: IsUserLoggedInRequest, res: Response) => {
      const blogId = req.params.id;
      if (!blogId)
        throw new ExpressError(
          "Blog ID is required in the request parameters.",
          400
        );
      const isLoggedIn = req.isLoggedIn;
      const userId = req.userId;
      ...
      if (!blog)
        throw new ExpressError("No blog found with the provided ID", 404);
      ...
      if (isLoggedIn && userId) {
        ...
        res
          .status(200)
          .json({
            message: "Blog with the provided ID found",
            data: blogWithFlag,
          });
      } else {
        res
          .status(200)
          .json({ message: "Blog with the provided ID found", data: blog });
      }
    }
  );
  ...
  static createBlog = asyncCatch(async (req: IAuthRequest, res: Response) => {
    ...
    if (
      ...
      throw new ExpressError("Enter the title or content", 400);
  ...
  static updateBlog = asyncCatch(async (req: IAuthRequest, res: Response) => {
    ...
    if (blogData.userId.toString() !== userId)
      throw new ExpressError("User cannot update other's blog.", 403);
  ...
  static deleteBlog = asyncCatch(async (req: IAuthRequest, res: Response) => {
    ...
    if (existingBlog.userId.toString() !== userId)
      throw new ExpressError("User cannot delete other's blog", 403);
